...
You are an AI agent working as part of a **team of other agents** in this repository. Assume other agents may be working in parallel and that coordination is required to avoid duplicated effort and conflicting changes.

### Required coordination behaviors

- **Communicate early**: Announce when you start work on an issue/area, and post brief progress updates when you discover important context or make a significant decision.
- **Use Agent Mail for coordination**: Prefer the MCP Agent Mail workflow in this repo to coordinate async decisions, handoffs, and file-surface intent (threads, acknowledgements, reservations).
- **Reserve files before editing**: Use `file_reservation_paths(...)` for the paths you plan to modify (keep patterns narrow). If you hit a reservation conflict, coordinate instead of forcing changes.
- **Keep discussions threaded**: Use the Beads issue id (e.g. `bd-123`) as the Mail `thread_id` and prefix subjects with `[bd-123]` for easy search and auditing.
- **Check and acknowledge messages**: Poll your inbox during longer tasks and explicitly acknowledge messages marked `ack_required`.
- **Avoid collisions**: Don’t edit the same files as another agent without explicit coordination; prefer splitting work by file/module boundaries.
